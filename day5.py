...
def check(update: list[int]) -> bool:
    for i, v in enumerate(update):
        prev, after_v = set(update[:i]), rules.get(v, set())
        if prev.intersection(after_v): return False
    return True

def get_mid(arr): return arr[len(arr) // 2]
...
def compare(x, y):
    ...
    if x in y_r and y in x_r: raise ValueError("unsortable!")
    if y in x_r: return -1
    if x in y_r: return 1
    return 0
